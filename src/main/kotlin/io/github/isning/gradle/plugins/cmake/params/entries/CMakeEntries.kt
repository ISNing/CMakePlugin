...
import io.github.isning.gradle.plugins.cmake.utils.*
    ...
    override val value: Map<String, String>
        get() = this::class.memberProperties.associate {
            when (it.name) {
                ...
                else -> propertyNameToKey[it.name] to it.getter.call(this)?.toString()
            }
        }.filter { it.key != null && it.value != null }.map { it.key!! to it.value!! }.toMap()

    val propertyNameToKey: Map<String, String>

    val CMakeParams.entriesByPropertyName: Map<String, String?>
        get() = propertyNameToKey.mapValues {
            entriesMap[it.value]
        }
}
...
val CMakeCacheEntries.asCMakeParams: CMakeParams
    get() = object : CMakeParams {
        override val value: List<String>
            get() = this@asCMakeParams.filteredValue.map { "-D${it.key}=${it.value}".warpQuotes() }
    }
...
val CMakeParams.entriesMap: Map<String, String>
    get() = filteredValue.findParameterValues("-D").parsePairs()
